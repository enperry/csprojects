...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
            ...
            if(people[person]["mother"] and people[person]["father"]):
                for parent in [people[person]["mother"], people[person]["father"]]:
                    if(genes[parent] == 2):
                        ...
                        prob = prob * 0.5 # mutations cancel out
                    else:
        ...
        # one gene in the scenario:
            ...
            # has parents:
            if(people[person]["mother"] and people[person]["father"]):
                probsum = genes[people[person]["mother"]] + genes[people[person]["father"]]
                if(probsum == 4 or probsum == 0):
                    prob = prob * (2 * PROBS["mutation"] * (1 - PROBS["mutation"]))
                elif(probsum == 3):
                    prob = prob * 0.5 # all mutations cancel out
                elif(probsum == 2):
                    if genes[people[person]["mother"]] == 1:
                        prob = prob * 0.5 # all mutations cancel out
                    else:
                        prob = prob * (1 - 2 * PROBS["mutation"] + 2 * PROBS["mutation"] ** 2)
        ...
        # traits:
        prob = prob * (PROBS["trait"][genes[person]][person in have_trait])
        joint = joint * prob
    ...
    for person in probabilities:
        for dist in probabilities[person]:
            ...
            for value in probabilities[person][dist]:
                probsum = probsum + probabilities[person][dist][value]
            for value in probabilities[person][dist]:
                probabilities[person][dist][value] /= probsum
